...
- 对 GitHub、Hugging Face、GitLab、Bitbucket、Wikipedia、Microsoft Learn 等站点支持直接抓取原始文本，无需经过 Jina。
...
- Wikipedia 词条页面（取其源文本）。
- Microsoft Learn 文档页面（取其 Markdown 版本）。

是否被识别为文本文件，取决于配置中列出的扩展名与文件名。其余站点的链接则通过 Jina Reader 读取，此时需要提供 Jina 密钥。
